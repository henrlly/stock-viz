...
view(Plot.plot({
  marks: data_ls.map((row) => {
    const data = row.arr;
    return [Plot.line(data.map((d) => ({[x]: d.timestep, [y]: d.forward_price})), {x: x, y: y, stroke: "green"}),
    Plot.tip(data.map((d) => ({[x]: d.timestep, [y]: d.forward_price})), Plot.pointer({x: x, y: y, maxRadius: 10, title: (d) => `${x}: ${d[x]}\n${y}: ${d[y]}\nSpot Price: ${row.sp}`})),] }
  )
}));
```
